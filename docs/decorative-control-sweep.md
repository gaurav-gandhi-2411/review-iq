...
| `render_metrics.py --check` | committed copy equals the JSON | marker blocks with a renderer; **was** silent on unknown names, unnamed/unterminated markers, missing target files | YES before (finding 3) | `pytest tests/unit/test_render_metrics.py tests/unit/test_render_metrics_markers.py`: 54 passed after (9 of them new, in `test_render_metrics_markers.py`); real repo: `OK` with one WARN naming `consensus_labeling` | fixed |
