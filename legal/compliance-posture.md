...
|---|---|---|
| Stateless-by-default processing (data minimization) | **Implemented and tested; not yet deployed to production** — the default mode for every organization persists no review text or extraction output anywhere (no database row, no application log field); retention is opt-in per organization with a chosen 30/90-day window and on-demand purge. Pending a schema migration (PR #167). | `docs/architecture/adr/0025-two-modes-retention-schema.md`, [`data-retention-and-deletion.md`](./data-retention-and-deletion.md) |
...
updated 2026-09-12 to add the stateless-by-default data-minimization control._
