...
| F — Reliability | **code done, no PR opened yet (per task instruction — orchestrator opens it)** | Headline finding: the org-key (paying-customer) extraction path had ZERO working failover — tiered-Groq exhaustion fell through past the plain-Groq-retry block (its condition requires tiered routing OFF) straight to `SecondaryProvider`, a literal `NotImplementedError` stub with no Cloud Run credentials configured either. Fixed: `SecondaryProvider` now makes a real OpenRouter call, restricted to Zero-Data-Retention endpoints via OpenRouter's documented per-request `provider.zdr=true` filter (not a blanket trust of "OpenRouter says so" — their own docs flag that upstream providers vary; the mechanism fails closed, verified live: a model with no ZDR endpoint gets HTTP 404, never silently routes to a training-eligible one). Live-verified 3x against `meta-llama/llama-3.3-70b-instruct` (DeepInfra, AkashML, DeepInfra — all cross-checked against `GET /api/v1/endpoints/zdr`'s live list for that model) plus one full live extraction round-trip through the real `complete()` method. `scripts/probe_failover.py`: nightly synthetic failover probe, real live calls (no mocks) against both Gemini-fallback and SecondaryProvider paths, exits 1 naming exactly which path broke; proved both directions live (both-OK run, both-FAIL run against an invalid model). Wired as `.github/workflows/failover-probe.yml` (cron, mirrors `eval.yml`'s nightly+Slack convention) — needs `GEMINI_API_KEY`/`SECONDARY_PROVIDER_API_KEY` GH secrets + `SECONDARY_PROVIDER_MODEL` repo variable added by GG (none exist yet; numbered steps in the report). Found `uptime-alert.yml` already exists (health probe every 5min → GH issue alert) — Task 3's gap is specifically a *public* status page, recommended UptimeRobot free tier (numbered signup steps for GG), not rebuilt in code. SLO: defined but explicitly NOT published as a measured percentage — real Cloud Monitoring data pulled live (99.972% non-5xx over 14d, ALL endpoints, not extraction-specific: `route` label is permanently empty on Cloud Run's own metric) and real Cloud Logging `llm.extracted` events show only **n=2 real extractions in 14 days** — far below any usable sample size, reported honestly as "INSUFFICIENT DATA" rather than fabricated. `scripts/slo_report.py` (new, $0 new deps — reuses already-transitive `google-auth`+`httpx`) makes this measurement repeatable once real traffic exists; wiring it to a recurring schedule needs Workload Identity Federation setup, flagged as a numbered escalation rather than silently added. Also fixed, found while touching CI: `ci.yml`'s "Unit tests" step only ran `tests/unit/`, silently excluding ~9 flat test files under `tests/` (including this section's own new SecondaryProvider tests) — widened after verifying all 1069 tests pass together. |
