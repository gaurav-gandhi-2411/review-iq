...
| E — Security + legal | in progress, PR #TBD (branch `feat/wave1-e-security-legal` pushed) | Legal docs done (6 files + REQUIRES-LEGAL-REVIEW banners + real DELETE /account documented, not the spec's non-existent DELETE /v2/data). CVE gate done (2 real HIGH CVEs found+fixed — pyasn1, pyjwt — blocking gate now live, all Actions SHA-pinned). Rate limiting confirmed already shipped (`/demo/extract` 5/min, signup 10/min, documented known multi-replica limitation, audit finding #10). Adversarial cross-tenant suite done: exact 4 spec vectors, all passing, plus a from-scratch RLS-disable proof (MVCC-safe technique, empirically verified before touching real tables). **Re-verified BYPASSRLS per the spec's explicit ask — it exists** (`review_iq_app` role, `rolbypassrls=true`), but investigation found it's an intentional, documented 2026-07-26 migration decision (matches Supabase's own `service_role` pattern; needed for admin.py's cross-org create/delete). Empirically confirmed `_set_tenant()`'s `SET LOCAL ROLE authenticated` genuinely re-enforces RLS despite the base connection's bypass. Audited every function in storage_pg.py: 20/21 correctly scope; the 1 exception is already explicitly justified in its own code comment. Added a static-audit regression test so a future unscoped function can't land silently. PII redaction rebuild (reversible token map, NER names, order-ID patterns, recall + accuracy-delta measurement) still in progress as of this checkpoint — see final report once it lands. |
