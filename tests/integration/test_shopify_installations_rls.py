"""RLS isolation proof for shopify_installations table.

Run AFTER applying 20260622000001_shopify_installations.sql.
Requires direct DB credentials (port 5432) in .env.

    uv run pytest tests/integration/test_shopify_installations_rls.py -v -m integration

WRITE PATH (OAuth callback):
    shopify_installations rows are written via service-role (postgres, no SET ROLE),
    exactly like org creation and API key provisioning in app/auth/signup.py.
    The org_id is resolved server-side from the authenticated seller's Supabase JWT:

        user  = verify_supabase_jwt(bearer_token)      # Supabase Admin SDK
        org   = _get_org_for_user(str(user.id))        # postgres-role lookup
        INSERT INTO shopify_installations (org_id=org["org_id"], shop_domain=..., ...)

    org_id is NEVER accepted as a user-controlled parameter. The only way a wrong
    org_id lands in the table is a bug in the callback's JWT verification path —
    which is tested separately in the OAuth callback unit tests (not yet built).
...
def org_ids() -> tuple[str, str]:
    ...
    try:
        ...
        cur.execute(
            ...
            "(%s, 'Shopify RLS Org A', %s), (%s, 'Shopify RLS Org B', %s)",
            (org_a, f"shopify-rls-a-{org_a[:8]}", org_b, f"shopify-rls-b-{org_b[:8]}"),
        )
        conn.commit()
    finally:
        conn.close()
...
def installation_ids(org_ids: tuple[str, str]) -> tuple[str, str]:
    ...
    """
    org_a, org_b = org_ids
    shop_a = f"store-rls-a-{org_a[:8]}.myshopify.com"
    shop_b = f"store-rls-b-{org_b[:8]}.myshopify.com"
    ...
    try:
        ...
        cur.execute(
            ...
            (org_a, shop_a, _FAKE_ENC_TOKEN, org_b, shop_b, _FAKE_ENC_TOKEN),
        )
        rows = cur.fetchall()
        conn.commit()
...
class TestShopifyInstallationsRLS:
    """Multi-tenant isolation proof for shopify_installations + webhook write scope."""
    ...
    def test_service_role_webhook_lookup_bypasses_rls(
        ...
        """Postgres role (no SET ROLE) sees ALL rows — mirrors _get_shopify_installation_pg."""
        org_a, org_b = org_ids
        inst_a, inst_b = installation_ids
        ...
        try:
            cur = conn.cursor()
            cur.execute(
                "SELECT id FROM public.shopify_installations WHERE id IN (%s, %s)",
                (inst_a, inst_b),
            )
            visible_ids = {str(row[0]) for row in cur.fetchall()}
        finally:
    ...
    def test_shop_domain_unique_prevents_duplicate_stores(
        ...
        unambiguous: at most one org_id per shop_domain, enforced at the DB level.
        """
        org_a, org_b = org_ids
        shop_a = f"store-rls-a-{org_a[:8]}.myshopify.com"
        ...
        try:
            ...
            with pytest.raises(psycopg2.errors.UniqueViolation):
                # Service-role INSERT attempting to steal shop_a for org_b
                cur.execute(
                    "INSERT INTO public.shopify_installations (org_id, shop_domain, access_token_enc) "
                    "VALUES (%s, %s, %s)",
                    (org_b, shop_a, _FAKE_ENC_TOKEN),
                )
            conn.rollback()
        finally:
            conn.close()

    def test_unrecognized_shop_domain_returns_no_row(self) -> None:
        """Webhook from a store that was never installed returns 0 rows — no fallback org."""
        conn = _conn()
        try:
            cur = conn.cursor()
            cur.execute(
                "SELECT org_id FROM public.shopify_installations "
                "WHERE shop_domain = %s AND revoked_at IS NULL",
                ("never-installed.myshopify.com",),
            )
            assert cur.fetchone() is None, "unrecognized shop must return no row"
        finally:
            conn.close()
    ...
    def test_webhook_lookup_and_extraction_scope(
        ...
        """Webhook routing chain: shop_a → org_a → extraction visible only to org A.

        Proves the full path that _process_webhook_review walks:
          1. X-Shopify-Shop-Domain (shop_a) → shopify_installations lookup → org_a_id
        ...
        This is the actual cross-tenant risk that SHOPIFY_WEBHOOK_API_KEY exposed:
        a webhook from store A writing into store B's org. This test proves that
        can't happen because the routing resolves org_a_id from the shop_domain,
        and the extraction is then RLS-scoped to that org_id.
        """
        org_a, org_b = org_ids
        shop_a = f"store-rls-a-{org_a[:8]}.myshopify.com"
        extraction_hash = f"shopify-scope-proof-{org_a[:8]}"

        # Step 1: simulate _get_shopify_installation_pg(shop_a) — postgres-role lookup.
        conn = _conn()
        try:
            cur = conn.cursor()
            cur.execute(
            ...
            row = cur.fetchone()
            assert row is not None, f"shop_a ({shop_a}) must resolve to an installation"
            resolved_org_id = str(row[0])
            assert resolved_org_id == org_a, (
                f"shop_a must resolve to org_a ({org_a}), got {resolved_org_id}"
            )
        finally:
            conn.close()
        ...
        # Step 3a: org A authenticated can see their Shopify-ingested extraction.
        conn = _conn()
        try:
            cur = conn.cursor()
            _set_tenant(cur, org_a)
            ...
            assert cur.fetchone()[0] == 1, (
                "org A must see the extraction ingested via their Shopify store"
            )
        finally:
            conn.close()

        # Step 3b: org B cannot see org A's Shopify-ingested extraction.
        conn = _conn()
        try:
            cur = conn.cursor()
            _set_tenant(cur, org_b)
