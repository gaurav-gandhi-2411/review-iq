...
tests instead prove the *wiring*: that the extraction funnel (cache-hit + fresh)
actually invokes it, that alert-layer failures never break ingestion, and that
daily_digest-configured event types still don't fire immediately from this path.

The POST /v2/authenticity and /v2/authenticity/batch call sites (and their wiring tests)
were removed with those routes in Session 15d (D5).

CSV ingest's authenticity-alert wiring is covered in
